...
class DHFFE(object):
    ...
    def __init__(self,MD,NMD):
        MD = np.asarray(MD)
        NMD = np.asarray(NMD)
        assert (MD.size==0 or NMD.size==0) or (max(MD)<=1 and max(NMD)<=1 and min(MD)>=0 and min(NMD)>=0) and (0<=max(MD)**3+max(NMD)**3<=1 and 0<=min(MD)**3+min(NMD)**3<=1),'ERROE:Construction failed! DHFFE max(MD)^3+max(NMD)^3 and min(MD)^3+min(NMD)^3 must be in interval[0,1]!'
        self.MD = MD
        self.NMD = NMD
    ...
    def Algebraic_Power(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = in_algebraic_tau(l*algebraic_tau(self.MD**3))**(1/3)
        newDHFE.NMD = in_algebraic_s(l*algebraic_s(self.NMD**3))**(1/3)
        return newDHFE
    ...
    def Algebraic_Times(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = in_algebraic_s(l*algebraic_s(self.MD**3))**(1/3)
        newDHFE.NMD = in_algebraic_tau(l*algebraic_tau(self.NMD**3))**(1/3)
        return newDHFE
    ...
    def Einstein_Power(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = in_einstein_tau(l*einstein_tau(self.MD**3))**(1/3)
        newDHFE.NMD = in_einstein_s(l*einstein_s(self.NMD**3))**(1/3)
        return newDHFE
    ...
    def Einstein_Times(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = in_einstein_s(l*einstein_s(self.MD**3))**(1/3)
        newDHFE.NMD = in_einstein_tau(l*einstein_tau(self.NMD**3))**(1/3)
        return newDHFE
    ...
    def Fast_Algebraic_Power(self,l):
        ...
        newDHFE.NMD = (1-(1-self.NMD**3)**l)**(1/3)
        return newDHFE
    ...
    def Fast_Algebraic_Times(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = (1-(1-self.MD**3)**l)**(1/3)
    ...
    def Fast_Einstein_Power(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = ((2*(self.MD**3)**l)/((2-self.MD**3)**l+(self.MD**3)**l))**(1/3)
        newDHFE.NMD = (((1+self.NMD**3)**l-(1-self.NMD**3)**l)/((1+self.NMD**3)**l+(1-self.NMD**3)**l))**(1/3)
        return newDHFE
    ...
    def Fast_Einstein_Times(self,l):
        ...
        '''
        newDHFE = DHFFE([],[])
        newDHFE.MD = (((1+self.MD**3)**l-(1-self.MD**3)**l)/((1+self.MD**3)**l+(1-self.MD**3)**l))**(1/3)
        newDHFE.NMD = ((2*(self.NMD**3)**l)/((2-self.NMD**3)**l+(self.NMD**3)**l))**(1/3)
        return newDHFE
...
def DHFFE_Intersection(dhfe1,dhfe2):
    ...
    assert dhfe1.qrung == dhfe2.qrung  and dhfe1.qrung==3,'ERROR! The two DHFEs are not the same DHFE or they may be not DHFFEs!'           ## 判断是否是同一种对偶犹豫模糊集
...
def DHFFE_Algebraic_Multiply(dhfe1,dhfe2):
    ...
    for md1 in dh_md1:
        for md2 in dh_md2:
            MD.append(pithy_algebraic_T(md1**3,md2**3)**(1/3))
    for nmd1 in dh_nmd1:
        for nmd2 in dh_nmd2:
            NMD.append(pithy_algebraic_S(nmd1**3,nmd2**3)**(1/3))
...
def DHFFE_Algebraic_Plus(dhfe1,dhfe2):
    ...
    for md1 in dh_md1:
        for md2 in dh_md2:
            MD.append(pithy_algebraic_S(md1**3,md2**3)**(1/3))
    for nmd1 in dh_nmd1:
        for nmd2 in dh_nmd2:
            NMD.append(pithy_algebraic_T(nmd1**3,nmd2**3)**(1/3))
...
def DHFFE_Einstein_Multiply(dhfe1,dhfe2):
    ...
    for md1 in dh_md1:
        for md2 in dh_md2:
            MD.append(pithy_einstein_T(md1**3,md2**3)**(1/3))
    for nmd1 in dh_nmd1:
        for nmd2 in dh_nmd2:
            NMD.append(pithy_einstein_S(nmd1**3,nmd2**3)**(1/3))
...
def DHFFE_Einstein_Plus(dhfe1,dhfe2):
    ...
    for md1 in dh_md1:
        for md2 in dh_md2:
            MD.append(pithy_einstein_S(md1**3,md2**3)**(1/3))
    for nmd1 in dh_nmd1:
        for nmd2 in dh_nmd2:
            NMD.append(pithy_einstein_T(nmd1**3,nmd2**3)**(1/3))
